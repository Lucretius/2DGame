...
public class EnemyWeak extends Mob {
	
	private boolean isAlive = true;
	private boolean justDied = false;
	private int color = Colors.get(-1, 000, 404, 543);
	private int deadColor = Colors.get(-1,000,300,404);
	private int scale = 1;
	private int absX = 0;
	private int absY = 0;
	...
	private Sound squeal = GameSounds.Squeal;
	private long timeOfDeath = 0L;
	private Player player = getClosestPlayer();

	
	public EnemyWeak(Level level, int x, int y) {
		super(level, "EnemyWeak", x, y, 1, 1,1);
		this.absX = x;
		this.absY = y;
		this.isAlive = true;
		this.id = id;
